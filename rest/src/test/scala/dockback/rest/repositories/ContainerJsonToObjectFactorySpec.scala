...
import dockback.domain.{Container, Image}
...
class ContainerJsonToObjectFactorySpec extends BaseSpec {
  ...
  feature( "Parse a one or more containers from a JSON string" ) {
    scenario("Parse a single container") {
      ...
      val container = ContainerJsonToObjectFactory.parseContainer( containerJson )

      Then("the container should be an container object")

      assert(container.isInstanceOf[Container])

      And( "the ID should be empty")
      assert(container.id.equals( "" ))
    ...
    scenario("parse an array of containers") {
      ...
      When("the string is parsed")
      val containers = ContainerJsonToObjectFactory.parseContainers( containersJson )

      Then( "the result should be a list of image objects" )
      assert(containers.isInstanceOf[java.util.List[Container]])
